...
class DatasetConfig(TypedDict):
    ...
    batch_size: int
    num_workers: int
    augmentation: Union[str, None] # AutoCIFAR10/CutMix/MixUp (concatenated with "-"")
...
def get_dataloader_cifar(
    ...
    norm = v2.Normalize((0.4915, 0.4823, 0.4468), (0.2470, 0.2435, 0.2616))
    ...
    if config["augmentation"] is not None and "AutoCIFAR10" in config["augmentation"]:
        train_transform = v2.Compose(
            [train_transform, v2.AutoAugment(v2.AutoAugmentPolicy.CIFAR10)]
        )

    if config["dataset"] == "CIFAR10":
        train_data = torchvision.datasets.CIFAR10(
            path, download=True, train=True, transform=train_transform
        )
        test_data = torchvision.datasets.CIFAR10(
            path, download=True, train=False, transform=test_transform
        )
        ...
        raise Exception(f"Unknown dataset {config['dataset']}")
...
def get_dataloader_imagenet(
    ...
    augment = (
        ...
        if config["augmentation"] is not None and "AutoImagenet" in config["augmentation"]
        else v2.Identity()
    )
    ...
    train_data = ImageNetDataset(
        path + "/ILSVRC/Data/CLS-LOC", split="train", transform=train_transform
    )
    test_data = ImageNetDataset(
        path + "/ILSVRC/Data/CLS-LOC", split="val", transform=test_transform
    )
...
def get_dataloader(config: DatasetConfig, path: str) -> tuple[DataLoader, DataLoader]:
    if config["dataset"] in ["CIFAR10", "CIFAR100"]:
        ...
        return get_dataloader_imagenet(config, path)
    else:
        raise Exception(f"Unknown dataset {config['dataset']}")
