...
class TrainConfig(TypedDict):
    ...
    autocast: bool
    lr_scheduler: str
    warmup_steps: Union[int, Iterable[int]]
...
def get_optim(config: TrainConfig, model: torch.nn.Module) -> torch.optim.Optimizer:
    ...
    if 'weight_decay' in config['optimizer']['args']:
        ...
        # optim_groups = [
        #     {'params': [param_map[name] for name in decay], 'weigth_decay': config['optimizer']['args']['weight_decay']},
...
def get_scheduler(config: TrainConfig, optim: torch.optim.Optimizer, **args) -> Union[torch.optim.lr_scheduler.LRScheduler, None]:
    if config['lr_scheduler'] == "like_transformer":
        warmup_steps = config['warmup_steps']
        assert isinstance(warmup_steps, int), "LR Scheduler expects 'warmup_steps' to be an int"
        lr_func = lambda step: args['d_model'] ** (-0.5) * min((step + 1) ** (-0.5), (step + 1) * warmup_steps ** (-1.5))
        return torch.optim.lr_scheduler.LambdaLR(optim, lr_func)
    elif config['lr_scheduler'] == 'warmup_cosine':
        warmup_steps = config['warmup_steps']
        assert isinstance(warmup_steps, int), "LR Scheduler expects 'warmup_steps' to be an int"
        linear = torch.optim.lr_scheduler.LinearLR(optim, 1/3, 1, total_iters=warmup_steps)
        cos = torch.optim.lr_scheduler.CosineAnnealingLR(optim, config['num_steps'] - warmup_steps, eta_min=1e-5)
        return torch.optim.lr_scheduler.SequentialLR(optim, [linear, cos], milestones=[warmup_steps])
    elif config['lr_scheduler'] == "constant":
        return None
    elif config['lr_scheduler'] == "step_decay":
        warmup_steps = config['warmup_steps']
        assert isinstance(warmup_steps, Iterable), "LR Scheduler expects 'warmup_steps' to be an Iterable of int"
        return torch.optim.lr_scheduler.MultiStepLR(optim, gamma=0.1, milestones=warmup_steps),
    elif config['lr_scheduler'] == "1cycle":
        return torch.optim.lr_scheduler.OneCycleLR(optim, max_lr=config['optimizer']['base_lr'], total_steps=config['num_steps'] + 1)
    else:
...
def train_loop(
        ...
        calc_train_loss: Callable[[torch.nn.Module, list[torch.Tensor], torch.device], torch.Tensor],
        train_loader: torch.utils.data.DataLoader,
        eval_model: Callable[[torch.nn.Module, torch.device], tuple[float, float, float, float]],
        device: torch.device,
    ...
    num_epochs = (config['num_steps'] // (len(train_loader) // config['batches_per_step'])) + 1
    ...
    for epoch in range(num_epochs):
            ...
            if (batch_index + 1) % config['batches_per_step'] == 0:
                ...
                if (step + 1) % config['log_interval'] == 0:
                    ...
                    if use_wandb:
                        ...
                        wandb.log({'loss': average_loss, 'learning_rate': current_lr}, step=step+1)
                    print(f'Epoch: {epoch + 1:3d}/{num_epochs:3d}, Step {step + 1:5d}/{config["num_steps"]}, Loss: {average_loss:.4f}')

                # After eval_every steps, calc accuracy
                if step % config['eval_interval'] == 0:
                    model.eval()
                    train_accuracy, train_entropy, test_accuracy, test_entropy = eval_model(model, device)
                    accuracy_curve.append(test_accuracy)
                    if use_wandb:
                        wandb.log({'accuracy': test_accuracy, 'train_accuracy': train_accuracy, 'entropy': test_entropy, 'train_entropy': train_entropy}, step=step+1)
                    print('=========================================')
                    print(f'Epoch: {epoch + 1:3d}/{num_epochs:3d}, Step: {step + 1:5d}/{config["num_steps"]}, Accuracy: {test_accuracy * 100:.4f} %')
                    print('=========================================')
        ...
        if epoch % 10 == 0:
            ...
            print(f'+++ Epoch {epoch + 1} took: {epoch_time}s, ETA: {timedelta(seconds=eta)}')
        start_time = time.time()
